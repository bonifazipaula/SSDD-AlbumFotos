...
// Función para generar thumbnail en cliente
  ...
  const processFile = async (file: File) => {
    ...
    if (!validation.valid) {
      setUploadingFiles((prev) =>
        prev.map((f) => (f.file === file ? { ...f, progress: "error", error: validation.error } : f)),
      )
      return
    }

    // Actualizar estado a uploading
    setUploadingFiles((prev) => prev.map((f) => (f.file === file ? { ...f, progress: "uploading" } : f)))
    ...
    try {
      ...
      // Marcar como éxito
      setUploadingFiles((prev) => prev.map((f) => (f.file === file ? { ...f, progress: "success" } : f)))
      ...
      setUploadingFiles((prev) =>
        prev.map((f) => (f.file === file ? { ...f, progress: "error", error: "Error al subir la imagen" } : f)),
      )
    }
  }
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          {isDragActive ? (
            ...
            <>
              <p className="text-foreground font-medium">Arrastra y suelta tus fotos aquí</p>
              <p className="text-sm text-muted-foreground">o haz clic para seleccionar archivos</p>
              <p className="text-xs text-muted-foreground mt-2">JPG, PNG, WebP - Máximo 10MB por imagen</p>
            </>
          )}
        </div>
      </div>
      ...
      {uploadingFiles.length > 0 && (
        <div className="space-y-2">
          {uploadingFiles.map((item, index) => (
            <div key={`${item.file.name}-${index}`} className="flex items-center gap-3 p-3 bg-muted rounded-lg">
              <div className="flex-shrink-0">
                {item.progress === "pending" && (
                  <div className="h-5 w-5 rounded-full border-2 border-muted-foreground" />
                )}
                {item.progress === "uploading" && <Loader2 className="h-5 w-5 animate-spin text-primary" />}
                {item.progress === "success" && <CheckCircle className="h-5 w-5 text-green-500" />}
                {item.progress === "error" && <AlertCircle className="h-5 w-5 text-destructive" />}
              </div>
              ...
              {(item.progress === "error" || item.progress === "pending") && (
                <Button variant="ghost" size="sm" onClick={() => removeFile(item.file)} className="flex-shrink-0">
                  <X className="h-4 w-4" />
                </Button>
              )}
            </div>
